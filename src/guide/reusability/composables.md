...
或者接收一個 [getter 函數](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/get#description)：
...
2. 偵聽器可以被關聯到該組件實例，以便在該組件被卸載時被銷毀，避免內存洩漏。
...
1. **不清晰的數據來源**：當使用了多個 mixin 時，實例上的數據屬性來自哪個 mixin 變得不清晰，這讓追溯實現和理解組件行為變得困難。這也是我們推薦在組合式函數中使用 ref + 解構模式的理由：讓屬性的來源在消費組件時一目了然。
