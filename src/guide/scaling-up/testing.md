...
- [Vitest](https://vitest.dev/)
  ...
  因為由 `create-vue` 創建的官方項目配置是基於 [Vite](https://vite.dev/) 的，所以我們推薦你使用一個可以使用同一套 Vite 配置和轉換管道的單元測試框架。[Vitest](https://vitest.dev/) 正是一個針對此目標設計的單元測試框架，它由 Vue / Vite 團隊成員開發和維護。在 Vite 的項目集成它會非常簡單，而且速度非常快。
...
> 你的測試越相似於你的軟件的使用方式，它們就越能值得你信賴。- [Kent C. Dodds](https://x.com/kentcdodds/status/977018512689455106) - Testing Library 的作者
...
- [Playwright](https://playwright.dev/) 是一個出色的端到端測試解決方案，支持 Chromium、WebKit 與 Firefox。它可以在 Windows、Linux、macOS 上運行，本地或在 CI 上，無頭或有頭模式皆可，並原生模擬 Android 上的 Google Chrome 與 Mobile Safari。它擁有信息豐富的界面、優秀的可調試性、內置斷言、並行化、追蹤功能，並致力於消除不穩定的測試。對[組件測試](https://playwright.dev/docs/test-components)的支持已經可用，但仍標記為實驗性。Playwright 是開源項目，由微軟維護。

- [Cypress](https://www.cypress.io/) 擁有信息豐富的圖形界面、優秀的可調試性、內置斷言、樁函數（stub）、抗剝落性以及快照。如上所述，它對[組件測試](https://docs.cypress.io/guides/component-testing/introduction)提供穩定支持。Cypress 支持基於 Chromium 的瀏覽器、Firefox 與 Electron。WebKit 支持已可用，但仍標記為實驗性。Cypress 採用 MIT 授權，但某些功能（如並行化）需要訂閱 Cypress Cloud。

<div class="lambdatest">
  <a href="https://lambdatest.com" target="_blank">
    <img src="/images/lambdatest.svg">
    <div>
      <div class="testing-partner">測試贊助商</div>
      <div>Lambdatest 是一個雲端平台，可在所有主流瀏覽器及真實裝置上運行端到端、無障礙與視覺回歸測試，並支持 AI 輔助生成測試！</div>
    </div>
  </a>
</div>
...
- [Nightwatch](https://nightwatchjs.org/) 是一個基於 [Selenium WebDriver](https://www.npmjs.com/package/selenium-webdriver) 的端到端測試解決方案。它的瀏覽器支持範圍最廣，包括原生的移動端測試。基於 Selenium 的解決方案會比 Playwright 或 Cypress 慢。
