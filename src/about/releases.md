...
小版本和大版本發佈通常會經歷一系列預發佈階段：**alpha**、**beta** 和 **候選發佈版 (release candidate, RC)**。預發佈的數量和類型取決於改動的範圍。例如，一個只包含少量更新的小版本可能僅有 beta 階段，而一個大版本通常會包含全部三個階段，以便進行充分的測試並接收社區反饋。

你可以通過 `npx install-vue@alpha`、`npx install-vue@beta` 或 `npx install-vue@rc` 從 npm 安裝最新的預發佈版本。若要測試尚未納入已標記預發佈版本中的改動，[vuejs/core](https://github.com/vuejs/core) 倉庫的每一次提交都會作為臨時的持續發佈預覽版進行發佈，你可以通過 `npx install-vue@edge` 安裝它。
