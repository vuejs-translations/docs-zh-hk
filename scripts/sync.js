...
async function getLatestSyncHash() {
  ...
  const latestSync = history.all
    .find((v) => /^sync #\w{7}\w?\s?$/i.test(v.body))
    ?.body?.trim()
  // console.log(history.all[0])
  if (!latestSync) {
    throw new Error(
      'No hash found. Are there any commit with `sync #hash` message in git history?'
    )
  }
...
async function getLatestUpstreamHash() {
  const git = SimpleGit()
  const latestUpstreamHash = await git.show([
    '-s',
    '--format="%h"',
    'origin/upstream'
  ])
...
program.name('sync').description('The sync helper CLI')
...
program
  ...
  .action(async () => {
    open(
      `https://github.com/vuejs/docs/compare/${await getLatestSyncHash()}...${await getLatestUpstreamHash()}`
    )
  })
...
program
  ...
  .option(
    '-o, --open',
    'directly open link to create pr (sync branch->main branch)'
  )
